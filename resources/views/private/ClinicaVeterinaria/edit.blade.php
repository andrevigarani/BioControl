...
@section('content')
    ...
    <div class="private-header">
        <h3 class="text-center">EDITAR CLÍNICA VETERINÁRIA</h3>
    </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-8 offset-md-2 p-5">
                    ...
                    <form action="{{ route('user.clinicasVeterinarias.update', $clinicaVeterina->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            ...
                            <select class="form-control" name="pessoa_juridica" required>
                                ...
                                @foreach ($pessoasJuridicas as $pessoaJuridica)
                                    <option value="{{ $pessoaJuridica->id }}" @if($clinicaVeterina->pessoaJuridica->id == $pessoaJuridica->id) selected @endif>{{ $pessoaJuridica->razao_social }} (CNPJ: {{ $pessoaJuridica->cnpj }})</option>
                                @endforeach
                            </select>
                        </div>
